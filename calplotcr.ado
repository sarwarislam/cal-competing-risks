...
program define calplotcr, rclass
...
*/
	...
	//get size of datavar
	qui sum id if `datavar_name' == `datavar_num'
	local obs = r(N)
	di `obs'
...
*/
		...
		forvalues k = 1/`K' {
			...
			if "`spikeplot'" == "dual" {
				qui twoway__histogram_gen `pred_c`k'' if _d == 0, frequency gen(`newvar'_c`k'_cens_bins `newvar'_c`k'_cens) width(`histwidth')
...
// 				}
// 				else {
// 					local height = `spikeheight'
// 				}
				qui replace `newvar'_c`k'_cens_bins = ((`newvar'_c`k'_cens_bins/`obs')*-`magnify') - `densaxis' 
			}   
			   
			qui twoway__histogram_gen `pred_c`k'' `ifd1', frequency gen(`newvar'_c`k'_event_bins `newvar'_c`k'_event) width(`histwidth')
...
// 					local height = `spikeheight'
// 				}
			qui replace `newvar'_c`k'_event_bins = ((`newvar'_c`k'_event_bins/`obs')*`magnify') - `densaxis'
...
/*
		* do for all events
		...
		if "`spikeplot'" == "dual" {
			...
			qui replace `newvar'_all_cens_bins = ((`newvar'_all_cens_bins/`height')*-`densaxis') - `densaxis' 
		}
		...
		qui replace `newvar'_all_event_bins = ((`newvar'_all_event_bins/`height')*`densaxis') - `densaxis'
...
// 				}
		...
		* do for all events
		...
		if "`spikeplot'" == "dual" {
			qui twoway__histogram_gen `pred_all' if _d == 0, frequency gen(`newvar'_all_cens_bins `newvar'_all_cens) width(`histwidth')
			local `newvar'_all_cens_width = r(width)
// 			qui summ `newvar'_all_cens_bins
// 			if `spikeheight' == 0 {
// 					local height = r(max)
...
// 					local height = `spikeheight'
// 				}
			qui replace `newvar'_all_cens_bins = ((`newvar'_all_cens_bins/`obs')*-`magnify') - `densaxis' 
		}
		   
		qui twoway__histogram_gen `pred_all' `ifd1', frequency gen(`newvar'_all_event_bins `newvar'_all_event) width(`histwidth')
		local `newvar'_all_event_width = r(width)
// 		qui summ `newvar'_all_event_bins
// 		if `spikeheight' == 0 {
// 					local height = r(max)
...
// 					local height = `spikeheight'
// 				}
		qui replace `newvar'_all_event_bins = ((`newvar'_all_event_bins/`obs')*`magnify') - `densaxis'
